...
void customQueue(const can_msgs::Frames::ConstPtr &msg){
  ROS_INFO_STREAM("Recived can msgs, Now send...");
...
void canSendThread(){
   ...
   ros::NodeHandle n;
   while (n.ok()){
     ROS_INFO_STREAM("FXXK THREAD");
     g_queue.callAvailable(ros::WallDuration(2));
   }
 }
...
int main(int argc, char **argv) {
  ...
  ros::SubscribeOptions ops = 
      ros::SubscribeOptions::create<can_msgs::Frames>("can_msgs",1000, customQueue,
                                                                                                                ros::VoidPtr(), &g_queue);
  ...
  ros::Publisher canMsgsPublisher = 
      nodeHandle.advertise<can_msgs::Frames> (myCanBridgeHandle.can_msgs_topic_name_,1);
