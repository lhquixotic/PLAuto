...
{
  private_node_handle_.param("input_topics", input_topics_, std::string("[/left/points_raw, /right/points_raw, /middle/points_raw]"));
  private_node_handle_.param("output_frame_id", output_frame_id_, std::string("rslidar"));
                                             ...
                                             const PointCloudMsgT::ConstPtr &msg7, const PointCloudMsgT::ConstPtr &msg8)
{
  ...
  try
  {
    for (size_t i = 0; i < input_topics_size_; ++i)
    {
      ...
      bool b = false;
      b = pcl_ros::transformPointCloud(output_frame_id_, ros::Time(0), *cloud_sources[i], msgs[i]->header.frame_id, *cloud_sources[i], tf_listener_);
      ROS_WARN_STREAM("b"<<b);
    }
  }
  ...
  // pointcloudXYZI => pointcloud2 ?
  // publsh points
  cloud_concatenated->header = pcl_conversions::toPCL(msgs[0]->header);
  cloud_concatenated->header.frame_id = output_frame_id_;
  
  PointCloudMsgT::Ptr cloud_concat(new PointCloudMsgT);
  pcl::toROSMsg(*cloud_concatenated,*cloud_concat);
  cloud_publisher_.publish(cloud_concat);
}
