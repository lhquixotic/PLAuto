...
{
  leader_distance_ =leader_speed_ = leader_acc_ = leader_acc_pedal_ 
          = leader_brake_pedal_ = leader_frontwheel_angle_ = 0;
  desired_distance_=10;
  kd=1;kv=1;ki=0.2;
  initForROS();
  health_checker_ptr_ =
...
void PurePursuitNode::initForROS()
{
  ...
  sub2_ = nh_.subscribe("/localization/utmpose", 10,
  ...
  sub4_ = nh_.subscribe("/chassis/vehicle_dynamic_state", 10,
    &PurePursuitNode::callbackFromCurrentVelocity, this);

  //todo change the topic name to v2v topic.
  sub5_ = nh_.subscribe("V2V",10, &PurePursuitNode::callbackFromCurrentVelocity, this);
...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    if (!is_pose_set_ || !is_waypoint_set_ || !is_velocity_set_)
    {
      ...
      ROS_WARN("pose:%d  waypoint:%d  velocity:%d",is_pose_set_,is_waypoint_set_,is_velocity_set_);
      loop_rate.sleep();
      continue;
    }
    // ROS_WARN("Success");
  ...
  const bool& can_get_curvature, const double& kappa) const
{
  ...
  ROS_INFO("command velocity:%f, command steering:%f",ccs.cmd.linear_velocity,ccs.cmd.steering_angle);
  pub2_.publish(ccs);
}
...
double PurePursuitNode::computeCommandVelocity() const
{
  ...
  double time_step=1.0/LOOP_RATE_;
  static double integral_value=0;
  integral_value+= (leader_distance_-desired_distance_ )*time_step;
  if(integral_value > 20) integral_value=20;
  else if(integral_value < -20) integral_value=-20;
  double command_velocity =  (leader_distance_-desired_distance_)*kd+(leader_speed_-current_linear_velocity_)*kv+ki*integral_value;
  // ROS_INFO("command velocity: %f",command_velocity);
...
void PurePursuitNode::callbackFromCurrentPose(const nav_msgs::OdometryConstPtr& msg)
{ 
  geometry_msgs::PoseStamped ps;
  ps.pose = msg -> pose.pose;
  pp_.setCurrentPose(ps);
  current_pose_ = ps.pose;
  is_pose_set_ = true;
}
...
{
  current_linear_velocity_ = msg->vehicle_speed;
  ...
  const autoware_msgs::LaneConstPtr& msg)
{
  if(msg->waypoints.size()>0)
    leader_distance_ = getPlaneDistance ( msg->waypoints.at( msg->waypoints.size()-1).pose.pose.position, current_pose_.position);
  else 
    leader_distance_ = 0;
...
void PurePursuitNode::callbackFromV2V(const common_msgs::V2VConstPtr &msg)
{
  ...
  leader_acc_pedal_ = msg->leader_acc_pedal;
  leader_brake_pedal_ = msg->leader_brake_pedal;
  leader_frontwheel_angle_ = msg->leader_frontwheel_angle;
